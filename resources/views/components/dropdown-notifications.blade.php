...
<div class="relative inline-flex" x-data="{ open: false }">
    <button
        class="flex items-center justify-center w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600/80"
    ...
    <div class="origin-top-right z-10 absolute top-full -mr-48 sm:mr-0 min-w-80 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-1.5 rounded shadow-lg overflow-hidden mt-1 {{ $align === 'right' ? 'right-0' : 'left-0' }}"
        ...
        <ul>
            <li class="border-b border-slate-200 dark:border-slate-700 last:border-0">
                <a class="block px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700/20" href="#0"
                    @click="open = false" @focus="open = true" @focusout="open = false">
                    <span class="block mb-2 text-sm">📣 <span class="font-medium text-slate-800 dark:text-slate-100">You
                            have a new test request</span> a patient.</span>
                    <span class="block text-xs font-medium text-slate-400 dark:text-slate-500">Aug 7, 2023</span>
                </a>
            </li>
        </ul>
    </div>

        ...
        <div class="flex">
            ...
            <div class="flex">
                <a href="{{ route('roles.index') }}"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-caret-left w-6 h-6 pt-2"
                        viewBox="0 0 16 16">
                        <path
            ...
            <div class="flex mt-2 border-t">
                ...
                <div class="px-4 py-2">
                    @foreach ($user->getRoleNames() as $role)
                        <span class="text-sm px-2 bg-gray-500 text-white rounded">{{ $role }}</span>
                    @endforeach
                </div>
            </div>
                ...
                <div class="px-4 py-2">
                    <ul class="grid grid-cols-3 gap-4">
                        @foreach ($user->getPermissionsViaRoles() as $permission)
                            <li class="bg-gray-500 mr-2 px-2 rounded mb-1 text-white text-sm">
                                {{ $permission->name }}
                            </li>
                        @endforeach
                    </ul>
